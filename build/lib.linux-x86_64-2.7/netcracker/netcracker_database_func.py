...
def accessDatabase(name="/usr/share/netcracker/database/netcracker.db"):
	db = Database(name)
...
def resetWordlistHistory(db, bssid):
	"""This function uses a db object and a string containing a bssid to reset wordlist history of cracking from a stored pcap file information."""
...
def createTables(dbObject):
	...
	SQL = ["CREATE TABLE IF NOT EXISTS HANDSHAKES (id INTEGER PRIMARY KEY AUTOINCREMENT, essid TEXT, bssid TEXT, file_location TEXT, file_capture TEXT, status TEXT, wordlists TEXT, password TEXT)",
...
def createWordlistTable(db, table_name):
	...
	if(table_name.upper() not in tables):
		...
		db.cursor.execute(QUERY1)
		QUERY2 = "CREATE TABLE IF NOT EXISTS %s (id INTEGER PRIMARY KEY AUTOINCREMENT, word TEXT)" % (table_name.upper())
...
def queryProbedEssids(db):
	"""This function uses a database object to return a list with all the probed ESSIDS from the PROBED_ESSIDS sql table."""
...
def insertProbedEssid(db, probed_essid, dbData):
	...
	if(addProbedEssid(db, probed_essid) != 0):
		...
		return -1
	print "[" +G+ "!" +W+ "] "+ "%sNew%s probed ESSID added to your %scollection%s: %s%s%s" % (G,W,G,W,YELLOW,probed_essid,W)
	return 0
...
def addPcapFile(db, pcap_file, ap_data):
	"""This function uses a db object file, a string of a pcap file, and a tuple (essid, bssid) to register a handshake into the handshake database."""
	essid = ap_data[0]
	bssid = ap_data[1]
	...
	if(bssid in [a[0] for a in fetch]):
		...
		return -1
	SQL = "INSERT INTO HANDSHAKES VALUES (NULL, '%s', '%s', '%s', '%s', 'UNCRACKED', NULL, NULL)" % (str(essid).replace("\n",""), str(bssid).replace("\n",""), pcap_file, str(time()))
	db.cursor.execute(SQL)
	db.handle.commit()
	return 0
